  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.type === 'workshop' && formData.workshop_date) {
        ...
        // Display will parse it as-is without conversion
      ...
      // Create recipients if not targeting all
      if (!formData.target_all && selectedTrainees.length > 0) {
      ...
      alert(locale === 'ar' ? 'تم إنشاء الإعلان بنجاح' : 'Announcement created successfully');
      router.push(`/${locale}/announcements`);
      router.refresh();
    } catch (error) {
      console.error('Error creating announcement:', error);
      alert(locale === 'ar' ? 'حدث خطأ أثناء إنشاء الإعلان' : 'Error creating announcement');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex gap-3 pt-6 border-t">
        ...
        >
          {loading
            ...
            : (locale === 'ar' ? 'نشر الإعلان' : 'Publish Announcement')}
        </button>
        <button
          type="button"
          onClick={() => router.back()}
